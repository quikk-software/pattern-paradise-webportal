...
export function ChatScreenComponent() {
  const [messages, setMessages] = useState<Message[]>([
    ...
    { id: 3, user: 'Charlie', content: 'Hello there!', timestamp: '10:02 AM' },
    {
      id: 4,
      user: 'Alice',
      content: 'I have a document to share.',
      timestamp: '10:03 AM',
      file: { name: 'document.pdf', type: 'document', url: '#' },
    },
    { id: 5, user: 'You', content: 'Thanks for sharing!', timestamp: '10:04 AM' },
    {
      ...
      timestamp: '10:05 AM',
      file: { name: 'image.jpg', type: 'image', url: '/placeholder.svg?height=200&width=200' },
    },
  ]);
      ...
      <CardContent className="p-6">
        <ScrollArea className="h-[600px] pr-4">
          {messages.map((message) => (
            <div
              key={message.id}
              className={`mb-4 ${message.user === 'You' ? 'ml-auto' : 'mr-auto'} max-w-[80%]`}
            >
              <div
                className={`flex items-start ${message.user === 'You' ? 'flex-row-reverse' : ''}`}
              >
                ...
                <div className={`flex-1 ${userColors[message.user]} rounded-lg p-3`}>
                  <div
                    className={`flex justify-between items-baseline ${message.user === 'You' ? 'flex-row-reverse' : ''}`}
                  >
                    <span className="font-semibold">{message.user}</span>
                    <span className="text-xs text-gray-500">{message.timestamp}</span>
                  </div>
                  <p className={`mt-1 ${message.user === 'You' ? 'text-right' : 'text-left'}`}>
                    {message.content}
                  </p>
                  {message.file && (
        ...
        {file && <div className="mt-2 text-sm text-gray-500">File selected: {file.name}</div>}
      </CardContent>
    </Card>
  );
}
